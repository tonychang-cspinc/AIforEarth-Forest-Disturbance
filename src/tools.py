...
def df_to_feature(gdf):
    features = []
    for g in gdf.geometry:
        if g.geom_type == 'MultiPolygon':
            for polygon in list(g):
                x,y = polygon.exterior.coords.xy
                cords = np.dstack((x,y)).tolist()
                p = ee.Geometry.Polygon(cords)
                feature = ee.Feature(p)
                features.append(feature)
    out_ee = ee.FeatureCollection(features)
    return out_ee
...
def get_points_centroid(df):
    '''Creates an EE geometry from set of points and returns the centroid'''
    lon = df.geometry.x
    lat = df.geometry.y 
    xmin = float(min(lon))
    xmax = float(max(lon))
...
def create_arraystack(featureStack, buffer=60):
    KERNEL_SIZE = buffer*2 
    lst = ee.List.repeat(1, KERNEL_SIZE)
    lists = ee.List.repeat(lst, KERNEL_SIZE)
    kernel = ee.Kernel.fixed(KERNEL_SIZE, KERNEL_SIZE, lists)
    arrays = featureStack.neighborhoodToArray(kernel)
    export_bands = [f['id'] for f in featureStack.getInfo()['bands']]
    return(arrays, export_bands)

def chunker(seq, size):
    return (seq[pos:pos + size] for pos in range(0, len(seq), size))

def create_subset(df, year):
    #subset = df[df['INVYR']==str(year)]
    subset = df[df['INVYR']==year]
    #subsetting here would also be by ecoregion...
    roi, center = get_points_centroid(subset)
    p = generate_ee_points(subset[['UNIQID', 'geometry']], idname='UNIQID')
    subset_pnts = p.toList(p.size())
    return(subset, roi, subset_pnts)

def get_landsat_data(bounds, datelist, \
    max_cloud_thr=None, \
    ...
    if month_aggregate:
        for month in range(1,13):
            ...
            monthly_mean = month_landsat.mean()
        ...
        if verbose:
            ...
            if  bandcount != 84:
                print('Warning!! LANDSAT data missing, check date range!\n')
        return out
        '''
        ...
        return landsat.select(band_selection).map(trimEdges)
...
def get_model_data(eegeom, datelist, monthwindow=1, max_cloud_thr=5, verbose=False):
    ...
    outdata = {'landsat':landsat,\
                 'climate':climate, 'topo':topo}#,\
                 #'naip':naip,'sentinel':sentinel
    if verbose:
